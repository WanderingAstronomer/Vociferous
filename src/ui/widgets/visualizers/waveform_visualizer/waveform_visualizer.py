...
class WaveformVisualizer(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # Colors
        self.bar_color_quiet = QColor(c.BLUE_5)
        self.bar_color_loud = QColor(c.BLUE_4)
    ...
    def add_spectrum(self, bands: list[float]) -> None:
        """
        Add spectrum data from audio input (adapter for BarSpectrumVisualizer API).

        Converts frequency spectrum to a single amplitude value by taking the
        maximum band value. This allows WaveformVisualizer to be a drop-in
        replacement for BarSpectrumVisualizer.

        Args:
            bands: List of normalized frequency band values (0.0 to 1.0)
        """
        if bands:
            amplitude = max(bands)
            self.add_level(amplitude)
