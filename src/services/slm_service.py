...
class SLMService(QObject):
    ...
    progress = pyqtSignal(str, str, int, int, bool) # job_id, stage, current, total, indeterminate
    ...
    def _get_gpu_memory_map(self) -> tuple[int, int] | None:
        ...
        try:
            ...
            result = subprocess.check_output(
                [smi_path, "--query-gpu=memory.total,memory.free", "--format=csv,noheader,nounits"],
                encoding="utf-8"
            )
    ...
    def _install_build_deps(self):
        ...
        self.statusMessage.emit("Installing build dependencies (torch, transformers)...")
    ...
    def _provision_model(self, cache_dir: Path, model_dir: Path):
        ...
        try:
            ...
            if source_tokenizer.exists():
                ...
                logger.warning("tokenizer.json not found in source, refinement might fail.")
    ...
    def initialize_service(self):
        ...
        try:
            ...
            if not self._validate_artifacts(model_dir):
                logger.info("Model artifacts missing or incomplete. Starting provisioning...")
            ...
            if mem_info:
                ...
                logger.info(f"GPU Info: Free={free_mb}MB, Total={total_mb}MB. Post-Load Headroom={headroom_ratio:.1%}")
                ...
                if headroom_ratio >= 0.40:
                    ...
                    logger.info("Low GPU headroom (<20%). Requesting user confirmation...")
                    ...
                    device = "cuda"
                    logger.info("Moderate GPU headroom (20-40%). Defaulting to CUDA for speed.")
    ...
    def handle_refinement_request(self, transcript_id: int, text: str, profile: str = "BALANCED"):
        ...
        if self.state != SLMState.READY:
             self.refinementError.emit(transcript_id, "Service preparing. Please wait...")
             return
