<script lang="ts">
  ...
  function highlight(text: string, q: string): string {
    ...
    return text.replace(new RegExp(`(${escaped})`, 'gi'), '<mark class="bg-[var(--color-accent)]/30 text-inherit rounded">$1</mark>');
  }
</script>
...
<div class="flex flex-col h-full p-6">
  ...
  <!-- Search bar -->
  <form class="flex gap-2 mb-6" onsubmit={(e) => { e.preventDefault(); handleSearch(); }}>
    <input
      type="text"
      bind:value={query}
  ...
  {:else if results.length > 0}
    <div class="text-xs text-[var(--color-text-muted)] mb-3">{results.length} result{results.length !== 1 ? 's' : ''}</div>
    <div class="flex-1 overflow-y-auto space-y-2">
      {#each results as entry (entry.id)}
        <div class="p-4 bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)] hover:border-[var(--color-accent)]/50 transition-colors">
          <p class="text-sm leading-relaxed">{@html highlight(entry.text || entry.raw_text, query)}</p>
