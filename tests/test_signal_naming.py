...
1. snake_case (no camelCase)
2. No hungarian notation ('Signal' suffix)
3. Event-based naming (verb_ed, noun_changed, noun_requested)
...
class TestSignalNaming:
    ...
    def test_signal_naming_conventions_in_icon_rail(self):
        """IconRail signals must use snake_case."""
    ...
    def test_signal_naming_conventions_in_main_window(self):
        """MainWindow signals must use snake_case."""
    ...
    def test_signal_naming_conventions_in_onboarding_pages(self):
        """Onboarding pages signals must use snake_case."""
    ...
    def _check_file_signals(self, filepath: Path):
        ...
        # snake_case pattern: lowercase, underscores allowed, no uppercase
            ...
            if isinstance(node, ast.Assign):
                ...
                if isinstance(node.value, ast.Call):
                    if (
                        ...
                        and node.value.func.id == "pyqtSignal"
                    ):
                        is_signal = True
                    elif (
                        isinstance(node.value.func, ast.Attribute)
                        and node.value.func.attr == "pyqtSignal"
                    ):
                        is_signal = True
                ...
                if is_signal:
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            ...
                            # Check 1: Must be snake_case
                            if not snake_case_pattern.match(name):
                                violations.append(f"{name} (must be snake_case)")
