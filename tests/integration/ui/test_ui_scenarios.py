...
def mock_history_manager():
    ...
    hm.get_entry.return_value = mock_entry
    hm.get_entry_by_timestamp.return_value = mock_entry
    return hm
...
def main_window(qapp, qtbot, mock_history_manager):
    ...
    with (
        patch(
            "src.database.history_manager.HistoryManager",
...
def test_transcribe_view_invariants(main_window, qtbot):
    ...
    assert btn_start.isVisible(), (
        "Start Recording button should be visible in IDLE state"
    )
    assert not btn_stop.isVisible(), (
        "Stop Recording button should be hidden in IDLE state"
    )
...
def test_edit_view_invariants(main_window, qtbot):
    ...
    if view:
        ...
        # Assert Action Dock has Save/Discard
        btn_save = find_action_button(main_window.action_dock, ActionId.SAVE)
        btn_discard = find_action_button(main_window.action_dock, ActionId.DISCARD)

        assert btn_save is not None
        assert btn_discard is not None

        # Validate logic: EditView.get_capabilities -> can_save=True, can_discard=True
        ...
        assert btn_discard.isVisible(), "Discard button should be visible in Edit View"
    else:
...
def test_projects_creation_invariant(main_window, qtbot):
    ...
    assert btn_create.isVisible(), (
        "Create Project button should be visible in Projects View"
    )
...
class TestActionDockButtonVisibilityInvariants:
    ...
    def test_recording_state_shows_stop_and_cancel_only(self, main_window, qtbot):
        ...
        assert not btn_start.isVisible(), (
            "Start Recording must be hidden during RECORDING"
        )
        assert btn_stop.isVisible(), "Stop Recording must be visible during RECORDING"
        assert btn_cancel.isVisible(), (
            "Cancel Recording must be visible during RECORDING"
        )
    ...
    def test_viewing_state_shows_full_action_set(self, main_window, qtbot):
        ...
        main_window.view_transcribe.workspace.load_transcript(
            "Test transcript text", "2025-01-01 12:00:00"
        )
        ...
        # Refine button visibility depends on refinement feature being enabled
        from src.core.config_manager import ConfigManager

        refinement_enabled = ConfigManager.get_config_value("refinement", "enabled")
        if refinement_enabled:
            assert btn_refine.isVisible(), (
                "Refine must be visible in VIEWING when refinement is enabled"
            )
        else:
            assert not btn_refine.isVisible(), (
                "Refine must be hidden in VIEWING when refinement is disabled"
            )
