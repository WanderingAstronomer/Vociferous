...
class TestTypeAnnotations:
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                for arg in node.args.args:
                    ...
                    if arg.annotation is None:
                        missing_annotations.append(f"{func_name} arg '{arg.arg}' missing annotation")

        if missing_annotations:
            pytest.fail(f"Type annotation errors in {filepath.name}:\n" + "\n".join(missing_annotations))
    ...
    def test_annotations_icon_rail(self):
        self._check_file_annotations(SRC_DIR / "ui" / "components" / "main_window" / "icon_rail.py")

    def test_annotations_main_window(self):
        self._check_file_annotations(SRC_DIR / "ui" / "components" / "main_window" / "main_window.py")
