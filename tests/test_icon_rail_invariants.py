...
from PyQt6.QtWidgets import QApplication, QWidget, QAbstractItemView, QListView, QTreeView

from ui.components.icon_rail import IconRail, RailButton
from ui.constants.view_ids import VIEW_TRANSCRIBE, VIEW_HISTORY, VIEW_PROJECTS
...
class TestIconRailInvariants:
    def test_canonical_composition_no_lists(self, qapp):
        ...
        for child in rail.findChildren(QWidget):
            if isinstance(child, banned_types):
                pytest.fail(f"Icon Rail contains banned widget type: {type(child).__name__}")
        
        # Also verify width is fixed (Invariant 7.2.3: Width is constant)
        assert rail.minimumWidth() == rail.maximumWidth()
        assert rail.width() == 64 # constant from code implies exact match to min/max
    ...
    def test_blink_signal_behavior(self, qapp, qtbot):
        ...
        # Spy on the signal
        with qtbot.waitSignal(target_btn.blinkFinished, timeout=1000):
            target_btn.blink()
