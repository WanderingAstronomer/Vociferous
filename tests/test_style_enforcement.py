...
# Permitted specific files (if any) - widgets with legacy inline colors
EXCLUDED_FILES = {
    COLORS_FILE.resolve(),
    (UI_ROOT / "widgets/dialogs/create_project_dialog.py").resolve(),
    (UI_ROOT / "widgets/toggle_switch.py").resolve(),  # Legacy widget
    (
        UI_ROOT
        / "widgets/visualizers/bar_spectrum_visualizer/bar_spectrum_visualizer.py"
    ).resolve(),  # Visualizer colors
}
...
def test_no_raw_colors_in_ui():
    ...
    assert not violations, (
        "Found hard-coded colors in UI files. Invariants restrict to ui.constants.Colors tokens:\n"
        + "\n".join(violations)
    )
...
def test_no_large_inline_stylesheets():
    ...
    for file_path in get_ui_files():
        if (
            file_path.resolve()
            == (UI_ROOT / "components/main_window/main_window.py").resolve()
        ):
            pass
    ...
    assert not violations, (
        "Found large inline stylesheets. Move styles to unified_stylesheet.py or *_styles.py:\n"
        + "\n".join(violations)
    )
...
def test_no_parallel_implementations():
    ...
    assert not violations, (
        "Found forbidden migration markers in UI files:\n" + "\n".join(violations)
    )
