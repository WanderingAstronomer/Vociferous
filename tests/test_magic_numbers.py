    ...
    def test_toggle_switch_constants(self):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                ...
                if isinstance(node.func, ast.Attribute) and node.func.attr == "setFixedSize":
                    for arg in node.args:
                        if isinstance(arg, ast.Constant) and isinstance(arg.value, int):
                            magic_numbers.append(f"setFixedSize({arg.value})")
                
                # Check for drawRoundedRect inputs
                if isinstance(node.func, ast.Attribute) and node.func.attr == "drawRoundedRect":
                     for arg in node.args:
                        if isinstance(arg, ast.Constant) and isinstance(arg.value, int):
        ...
        found_targets = [m for m in magic_numbers if any(t in m for t in target_literals)]
