...
def test_view_host_routing_signals(qapp):
    ...
    # ViewHost now always emits viewChanged to ensure observers sync,
    # even when switching to the already-current view
    host.switch_to_view("v1")
    assert signals == ["v1"], "ViewHost emits signal on switch_to_view, even if already current"
...
def test_view_host_initial_state_deterministic(qapp):
    ...
    assert host.get_current_view_id() == "initial_view", "First registered view must be active by default"
    
    host.register_view(v2, "second_view")
    assert host.get_current_view_id() == "initial_view", "Registration of subsequent views must not change active view"
